"""Script simplificat pentru vizualizare rezultate"""
...
def load_results(results_dir):
    """Incarca rezultatele din fisierul JSON."""
    results_file = os.path.join(results_dir, 'results.json')
    with open(results_file, 'r') as f:
        return json.load(f)

def smooth_curve(values, weight=0.9):
    """Neteaza o curba folosind exponential moving average."""
    smoothed = []
    last = values[0]
    for value in values:
...
def plot_learning_curves(results, save_dir):
    """Ploteaza curbele de invatare."""
    fig, axes = plt.subplots(2, 2, figsize=(16, 12))
    fig.suptitle('Learning Curves - Algorithm Comparison', fontsize=16, fontweight='bold')
...
def plot_final_comparison(results, save_dir):
    """Ploteaza comparatie finala."""
    fig, axes = plt.subplots(1, 2, figsize=(14, 6))
    fig.suptitle('Final Comparison between Algorithms', fontsize=16, fontweight='bold')
